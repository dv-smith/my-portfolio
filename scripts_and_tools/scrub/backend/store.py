...
def init_db():
    ...
    # Custom keywords — per-engagement sensitive terms
    # e.g. client app names, internal hostnames, service account prefixes
    cur.execute("""
...
def add_custom_keyword(engagement_id: str, keyword: str) -> bool:
    """Add a custom sensitive keyword for an engagement. Returns True if added, False if duplicate."""
    ...
    try:
        cur.execute(
            "INSERT OR IGNORE INTO custom_keywords (engagement_id, keyword, created_at) VALUES (?,?,?)",
            (engagement_id, keyword, time.time())
        )
...
def get_custom_keywords(engagement_id: str) -> list[str]:
    """Return all custom keywords for an engagement."""
    conn = sqlite3.connect(DB_PATH)
    cur  = conn.cursor()
    cur.execute(
        "SELECT keyword FROM custom_keywords WHERE engagement_id = ? ORDER BY created_at",
        (engagement_id,)
    )
...
def delete_custom_keyword(engagement_id: str, keyword: str) -> bool:
    """Delete a custom keyword. Returns True if deleted."""
    conn = sqlite3.connect(DB_PATH)
    cur  = conn.cursor()
    cur.execute(
        "DELETE FROM custom_keywords WHERE engagement_id = ? AND keyword = ?",
