...
async def sanitise(req: SanitiseRequest):
    ...
    # Load per-engagement custom keywords and pass to pipeline
    keywords = get_custom_keywords(req.engagement_id)

    # Run the pipeline
    result: PipelineResult = run_pipeline(req.text, salt, custom_keywords=keywords)
...
@app.get("/api/engagement/{engagement_id}/keywords")
async def list_keywords(engagement_id: str):
    """List custom sensitive keywords for an engagement."""
    return {"engagement_id": engagement_id, "keywords": get_custom_keywords(engagement_id)}


@app.post("/api/engagement/{engagement_id}/keywords")
async def add_keyword(engagement_id: str, body: dict):
    """Add a custom sensitive keyword for an engagement."""
    keyword = (body.get("keyword") or "").strip()
    if not keyword or len(keyword) < 2:
        raise HTTPException(status_code=400, detail="keyword must be at least 2 characters")
    added = add_custom_keyword(engagement_id, keyword)
    return {"engagement_id": engagement_id, "keyword": keyword, "added": added}


@app.delete("/api/engagement/{engagement_id}/keywords/{keyword}")
async def remove_keyword(engagement_id: str, keyword: str):
